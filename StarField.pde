public class StarField implements Visual{
  Minim minim;
  AudioInput in;
  
  // I create an array named "stars",
  // it will be filled with 1600 elements made with the Star() class.
  Star[] stars = new Star[1600];

  // I create a variable "speed", it'll be useful to control the speed of stars.
  float speed;
  float speedVolumeIncrease = 50;
  
  StarField(Minim minim, AudioInput in){
    this.minim = minim;
    this.in = in;
  }
  ...
  void drawVisual() {
    speed = 1 + in.mix.level() * speedVolumeIncrease;
    colorMode(HSB);
    background(0);
